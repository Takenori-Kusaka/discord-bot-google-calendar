...
class GoogleCalendarAPI:
    ...
    def initialize_credentials(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Google Calendar API: {e}", exc_info=True
            )
            if not self.test_mode:
                raise
            logger.warning(
                "Running in test mode - continuing without Google Calendar API"
            )
    ...
    def get_events(self, max_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            formatted_events = [
                self.format_event_for_display(event) for event in events
            ]

        ...
        try:
            ...
            if days_diff <= 1:
                ...
                for i in range(len(hourly.Variables(0).ValuesAsNumpy())):
                    hour_data = {
                        "time": datetime.fromtimestamp(hourly.Time()[i], self.timezone).strftime("%H:%M"),
                        "temperature": round(hourly.Variables(0).ValuesAsNumpy()[i], 1),
                        "humidity": round(hourly.Variables(1).ValuesAsNumpy()[i]),
                        "precipitation_prob": round(hourly.Variables(2).ValuesAsNumpy()[i]),
                        "weather": self.weathercode_mapping.get(
                            int(hourly.Variables(3).ValuesAsNumpy()[i]), "不明"
                        ),
                        "apparent_temp": round(hourly.Variables(4).ValuesAsNumpy()[i], 1),
                    }
                ...
                result.update({
                    "today": {
                        ...
                        "precipitation_sum": round(daily.Variables(2).ValuesAsNumpy()[0], 1),
                        "weather": self.weathercode_mapping.get(
                ...
                for i in range(len(daily.Variables(0).ValuesAsNumpy())):
                    day_data = {
                        "date": datetime.fromtimestamp(daily.Time()[i], self.timezone).strftime("%Y-%m-%d"),
                        "max_temp": round(daily.Variables(0).ValuesAsNumpy()[i], 1),
                        "min_temp": round(daily.Variables(1).ValuesAsNumpy()[i], 1),
                        "precipitation_sum": round(daily.Variables(2).ValuesAsNumpy()[i], 1),
                        "weather": self.weathercode_mapping.get(
                            int(daily.Variables(3).ValuesAsNumpy()[i]), "不明"
                        ),
                        "precipitation_prob": round(daily.Variables(4).ValuesAsNumpy()[i]),
                    }

...
class AnthropicAgent(Agent):
    ...
    def __init__(self, name: str, description: str, system_prompt: str):
        ...
        self.logger.debug(
            f"Initializing Anthropic client with API key: {api_key[:6]}..."
        )
    ...
    async def process(self, input_text: str) -> AgentResponse:
        ...
        try:
            ...
            response_text = (
                message.content[0].text if message.content else "応答がありません。"
            )
            self.logger.debug(f"Received response: {response_text}")
            return AgentResponse(agent_name=self.name, response=response_text)
...
async def main():
    ...
    if service_account_json:
        logger.debug(
            "GOOGLE_SERVICE_ACCOUNT_JSON length: %d", len(service_account_json)
        )
        logger.debug(
            "GOOGLE_SERVICE_ACCOUNT_JSON first 100 chars: %s",
            service_account_json[:100],
        )
    else:
    ...
    config = OrchestratorConfiguration(
        agents=[butler_agent, calendar_agent], default_agent_name=butler_agent.name
    )
    ...
    while True:
        ...
        try:
            ...
            if any(
                word in user_input
                for word in ["予定", "スケジュール", "打ち合わせ", "ミーティング"]
            ):
                logger.debug(
                    "Calendar-related input detected, processing with calendar agent"
                )

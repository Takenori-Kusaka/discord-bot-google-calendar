...
async def test_calendar_agent_add_event(mock_initialize, mock_anthropic, calendar_agent):
    # Anthropic APIのモック設定
    ...
    mock_service.events().insert().execute.return_value = {'htmlLink': 'http://example.com'}
...
async def test_parse_datetime_variations(mock_anthropic, calendar_agent):
    ...
    test_cases = [
        # 基本的な時間指定
        {
            "input": "明日の10時から11時まで",
            "json_response": {
                "start_time": {"year": 2025, "month": 1, "day": 20, "hour": 10, "minute": 0},
                "end_time": {"year": 2025, "month": 1, "day": 20, "hour": 11, "minute": 0}
            }
        },
        # 午前/午後の指定
        {
            "input": "今日の午後3時から5時まで",
            "json_response": {
                "start_time": {"year": 2025, "month": 1, "day": 19, "hour": 15, "minute": 0},
                "end_time": {"year": 2025, "month": 1, "day": 19, "hour": 17, "minute": 0}
            }
        },
        # 分の指定あり
        {
            ...
            "json_response": {
                "start_time": {"year": 2025, "month": 1, "day": 20, "hour": 10, "minute": 30},
                "end_time": {"year": 2025, "month": 1, "day": 20, "hour": 11, "minute": 45}
            }
        }
    ]
